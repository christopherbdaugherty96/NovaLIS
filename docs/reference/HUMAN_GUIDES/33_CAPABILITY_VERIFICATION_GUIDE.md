...
Once all 5 earlier phases pass, the capability can be locked:
...
From that point on, the regression guard runs on every CI invocation and blocks if those fields have silently changed.
...
- no capability is marked `locked: true` unless all 5 earlier phases are `pass`
- locked capabilities' governance fields match the snapshot taken at lock time

If a developer changes a locked capability's `authority_class` or `risk_level` without first unlocking it, this guard will fail CI.
...
Capability 64 (`send_email_draft`) is the highest priority because it is the first capability that reaches outside the local machine by opening the system mail client.
...
As of 2026-04-18:
- Cap 64 (`send_email_draft`): P1 pass, P2 pass, P3 pass, P4 pass, P5 pending
