...
class InputNormalizer:
    ...
    PHRASE_NORMALIZATION = (
        ...
        (r"^\s*what(?:'s| is)\s+the weather\s+like(?:\s+today)?\s+in\s+.+$", "weather"),
        (r"^\s*what(?:'s| is)\s+the weather\s+like(?:\s+today)?\s*$", "weather"),
        ...
        (r"^\s*(?:give me|show me)\s+(?:the\s+)?(?:latest|current|recent|top)\s+(?:news\s+)?headlines?\s*$", "news"),
        (r"^\s*(?:what\s+are|show me)\s+(?:the\s+)?(?:latest|current|recent|top)\s+(?:news\s+)?headlines?\s*$", "news"),
        (r"^\s*(?:latest|current|recent|top)\s+(?:news\s+)?headlines?\s*$", "news"),
