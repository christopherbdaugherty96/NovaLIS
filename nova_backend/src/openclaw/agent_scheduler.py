...
class OpenClawAgentScheduler:
    ...
    def __init__(self, *, store: OpenClawAgentRuntimeStore | None = None, runner: OpenClawAgentRunner | None = None, settings: RuntimeSettingsStore | None = None, notification_policy_store: NotificationScheduleStore | None = None, ledger_logger: LedgerLogger | None = None) -> None:
    ...
    async def tick(self, *, now: datetime | None = None) -> list[dict[str, Any]]:
        ...
        for claim in due_templates:
            ...
            if template is None:
                try:
                    self._store.record_scheduled_run_outcome(template_id, outcome="failed", note="template_not_found: store returned None", now=now)
                except Exception:
                    pass
                self._log("OPENCLAW_AGENT_SCHEDULE_FAILED", {"template_id": template_id, "error": "template_not_found: store returned None", "source": "agent_scheduler"})
                continue
            feature_on = os.getenv(_FEATURE_FLAG_ENV, "").strip().lower() in {"1", "true", "yes"}
            if feature_on:
                try:
                    factory = EnvelopeFactory(runtime_settings=self._settings)
                    issued = factory.issue(template=template, channel="scheduler", triggered_by="scheduler")
                    EnvelopeStore().register(envelope_id=issued.envelope_id, envelope_data=issued.envelope.to_dict(), issuing_channel=issued.issuing_channel, settings_hash=issued.settings_hash, feature_flags_snapshot=issued.feature_flags_snapshot, issued_at=issued.issued_at, expires_at=issued.expires_at)
            ...
            try:
                ...
                self._log("OPENCLAW_AGENT_SCHEDULE_FAILED", {"template_id": template_id, "error": str(exc), "source": "agent_scheduler"})
        return completed
