...
from nova_backend.tests.phase45.dashboard_event_replay_harness import DashboardEventReplayHarness
...
def test_replay_blocks_double_submit_without_second_payload():
    ...
    assert replay.loading_hint == "Nova is still answering. Give this turn a moment before sending another."
...
def test_replay_unsupported_event_is_visible_non_action_state():
    ...
    assert replay.assistant_messages[0].confidence == "Unsupported"
    assert "Nova did not treat it as success or execute anything." in replay.assistant_messages[0].text
...
def test_dashboard_event_replay_harness_is_anchored_to_source_contracts():
    ...
    for expected in (
        ...
        "Nova is still answering. Give this turn a moment before sending another.",
