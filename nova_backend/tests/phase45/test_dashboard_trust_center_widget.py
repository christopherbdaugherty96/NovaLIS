...
def test_dashboard_renders_trust_center_page_from_runtime_state():
    ...
    assert "Home Agent helps Nova run manual tasks you can review and stop." in source
    assert "function loadConnectionsData()" in source
    assert "function renderConnectionCards()" in source
    ...
    assert "connection-cards-grid" in source
