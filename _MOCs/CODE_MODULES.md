---
tags:
  - moc
  - nova
  - code
  - modules
---
<!-- GENERATED BY scripts/generate_obsidian_overlay.py - DO NOT EDIT: re-run the script to refresh. -->

# Code modules (nova_backend/src)

One section per top-level module under `nova_backend/src/`. Each section lists
its files, the internal modules it imports, the modules that import it, and
tests or docs that match by name or topic.

## `actions` (4 files)

### Files

- [[nova_backend/src/actions/__init__.py|__init__ - src/actions]]
- [[nova_backend/src/actions/action_request.py|action_request]]
    - @dataclass(frozen=True)
- [[nova_backend/src/actions/action_result.py|action_result]]
    - @dataclass
- [[nova_backend/src/actions/action_types.py|action_types]]
    - class ActionType(str, Enum):

### Imported by

- [[nova_backend/src/archive_quarantine/phase2_tests/phase2_manual_tests.py|phase2_manual_tests]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/analysis_document_executor.py|analysis_document_executor]]
- [[nova_backend/src/executors/brightness_executor.py|brightness_executor]]
- [[nova_backend/src/executors/explain_anything_executor.py|explain_anything_executor]]
- [[nova_backend/src/executors/external_reasoning_executor.py|external_reasoning_executor]]
- [[nova_backend/src/executors/info_snapshot_executor.py|info_snapshot_executor]]
- [[nova_backend/src/executors/media_executor.py|media_executor]]
- [[nova_backend/src/executors/memory_governance_executor.py|memory_governance_executor]]
- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]]
- [[nova_backend/src/executors/news_intelligence_executor.py|news_intelligence_executor]]
- [[nova_backend/src/executors/open_folder_executor.py|open_folder_executor]]
- [[nova_backend/src/executors/openclaw_execute_executor.py|openclaw_execute_executor]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/executors/response_verification_executor.py|response_verification_executor]]
- _…and 29 more_

## `agents` (11 files)

___all__ = [_

### Files

- [[nova_backend/src/agents/__init__.py|__init__ - src/agents]]
- [[nova_backend/src/agents/_llm_agent.py|_llm_agent]]
    - class LLMBackedAgent(BaseAgent):
- [[nova_backend/src/agents/adversarial.py|adversarial]]
    - class AdversarialExternalizerAgent(LLMBackedAgent):
- [[nova_backend/src/agents/architect.py|architect]]
    - class StructuralArchitectAgent(LLMBackedAgent):
- [[nova_backend/src/agents/assumption.py|assumption]]
    - class AssumptionRiskAgent(LLMBackedAgent):
- [[nova_backend/src/agents/base.py|base]]
    - @dataclass(frozen=True)
- [[nova_backend/src/agents/builder.py|builder]]
    - class BuilderAgent(LLMBackedAgent):
- [[nova_backend/src/agents/context.py|context]]
    - def _freeze(value: Any) -> Any:
- [[nova_backend/src/agents/contradiction.py|contradiction]]
    - class ContradictionAggregatorAgent(LLMBackedAgent):
- [[nova_backend/src/agents/deep_audit.py|deep_audit]]
    - class DeepAuditAgent(LLMBackedAgent):
- [[nova_backend/src/agents/memory.py|memory - src/agents]]
    - class MemoryAgent(LLMBackedAgent):

### Imports from

- [[nova_backend/src/llm/llm_gateway.py|llm_gateway]] (`src.llm.llm_gateway`)

### Imported by

- [[nova_backend/src/governor/agent_orchestrator.py|agent_orchestrator - src/governor]]
- [[nova_backend/src/personality/core.py|core]]
- [[nova_backend/src/personality/presenter.py|presenter]]
- [[nova_backend/src/websocket/intent_patterns.py|intent_patterns]]
- [[nova_backend/tests/phase42/test_context_snapshot.py|test_context_snapshot]]
- [[nova_backend/tests/phase42/test_phase42_orchestrator_context.py|test_phase42_orchestrator_context]]
- [[nova_backend/tests/phase42/test_phase42_runtime_lock.py|test_phase42_runtime_lock]]

## `api` (10 files)

_Focused FastAPI route modules for Nova runtime surfaces._

### Files

- [[nova_backend/src/api/__init__.py|__init__ - src/api]]
- [[nova_backend/src/api/audit_api.py|audit_api]]
    - def build_audit_router(deps) -> APIRouter:
- [[nova_backend/src/api/bridge_api.py|bridge_api]]
    - BRIDGE_ALLOWED_CAPABILITY_IDS = frozenset(
- [[nova_backend/src/api/connections_api.py|connections_api]]
    - Connection cards API.
- [[nova_backend/src/api/live_screen_api.py|live_screen_api]]
    - log = logging.getLogger("nova.live_screen")
- [[nova_backend/src/api/memory_api.py|memory_api]]
    - def build_memory_router(deps) -> APIRouter:
- [[nova_backend/src/api/openclaw_agent_api.py|openclaw_agent_api]]
    - def _notification_policy_snapshot(deps) -> dict[str, Any]:
- [[nova_backend/src/api/profile_api.py|profile_api]]
    - def build_profile_router(deps: Any) -> APIRouter:
- [[nova_backend/src/api/settings_api.py|settings_api]]
    - def _sync_usage_budget(settings_snapshot: dict[str, Any]) -> None:
- [[nova_backend/src/api/workspace_api.py|workspace_api]]
    - def build_workspace_router(deps) -> APIRouter:

### Imports from

- [[nova_backend/src/connections/connections_store.py|connections_store]] (`src.connections.connections_store`)
- [[nova_backend/src/governor/execute_boundary/__init__.py|__init__ - governor/execute_boundary]] (`src.governor.execute_boundary`)
- [[nova_backend/src/governor/governor_mediator.py|governor_mediator - src/governor]] (`src.governor.governor_mediator`)
- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/llm/llm_gateway.py|llm_gateway]] (`src.llm.llm_gateway`)
- [[nova_backend/src/memory/governed_memory_store.py|governed_memory_store]] (`src.memory.governed_memory_store`)
- [[nova_backend/src/memory/nova_self_memory_store.py|nova_self_memory_store]] (`src.memory.nova_self_memory_store`)
- [[nova_backend/src/memory/user_memory_store.py|user_memory_store]] (`src.memory.user_memory_store`)
- [[nova_backend/src/perception/ocr_pipeline.py|ocr_pipeline]] (`src.perception.ocr_pipeline`)
- [[nova_backend/src/perception/vision_analyzer.py|vision_analyzer]] (`src.perception.vision_analyzer`)
- [[nova_backend/src/profiles/user_profile_store.py|user_profile_store]] (`src.profiles.user_profile_store`)
- [[nova_backend/src/skills/calendar.py|calendar]] (`src.skills.calendar`)
- [[nova_backend/src/tasks/notification_schedule_store.py|notification_schedule_store]] (`src.tasks.notification_schedule_store`)
- [[nova_backend/src/usage/provider_usage_store.py|provider_usage_store]] (`src.usage.provider_usage_store`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/tests/phase45/test_connections_api.py|test_connections_api]]
- [[nova_backend/tests/phase45/test_profile_api.py|test_profile_api]]
- [[nova_backend/tests/test_live_screen_api.py|test_live_screen_api]]

### Tests

- [[nova_backend/tests/phase45/test_connections_api.py|test_connections_api]]
- [[nova_backend/tests/phase45/test_profile_api.py|test_profile_api]]
- [[nova_backend/tests/test_audit_api.py|test_audit_api]]
- [[nova_backend/tests/test_live_screen_api.py|test_live_screen_api]]
- [[nova_backend/tests/test_memory_api.py|test_memory_api]]
- [[nova_backend/tests/test_openclaw_agent_api.py|test_openclaw_agent_api]]
- [[nova_backend/tests/test_workspace_api.py|test_workspace_api]]

## `archive_quarantine` (14 files)

### Files

- [[nova_backend/src/archive_quarantine/phase2_execution/brain_safeP1-2.5.py|brain_safeP1-2.5]]
    - NovaLIS Brain Server — Phase-2 Baseline
- [[nova_backend/src/archive_quarantine/phase2_governo/governor_mediation.py|governor_mediation - archive_quarantine/phase2_governo]]
    - NOVALIS GOVERNOR MEDIATION LAYER
- [[nova_backend/src/archive_quarantine/phase2_tests/phase2_governor_tests.py|phase2_governor_tests]]
    - phase2_governor_tests.py
- [[nova_backend/src/archive_quarantine/phase2_tests/phase2_manual_tests.py|phase2_manual_tests]]
    - def run(name, fn):
- [[nova_backend/src/archive_quarantine/phase35_execution/execution/__init__.py|__init__ - phase35_execution/execution]]
- [[nova_backend/src/archive_quarantine/phase35_execution/execution/execute_action.py|execute_action]]
    - NovaLIS Action Execution Boundary — GOVERNOR PRIVATE
- [[nova_backend/src/archive_quarantine/phase35_execution/execution/handlers/open_app.py|open_app]]
    - NovaLIS — Phase-2 Handler: open_app
- [[nova_backend/src/archive_quarantine/phase35_execution/execution/handlers/open_folder.py|open_folder]]
    - NovaLIS — Phase-2 Handler: open_folder
- [[nova_backend/src/archive_quarantine/phase35_execution/execution/handlers/open_view.py|open_view]]
    - NovaLIS — Phase-2 Handler: open_view
- [[nova_backend/src/archive_quarantine/phase35_execution/execution/handlers/volume_control.py|volume_control]]
    - NovaLIS — Phase-2 Handler: volume_control
- [[nova_backend/src/archive_quarantine/phase35_execution/executors/__init__.py|__init__ - phase35_execution/executors]]
- [[nova_backend/src/archive_quarantine/phase35_execution/executors/executor_registry.py|executor_registry]]
    - Executor Registry (Phase-2 Scaffold)
- [[nova_backend/src/archive_quarantine/phase35_execution/executors/executor_types.py|executor_types]]
    - Executor Types
- [[nova_backend/src/archive_quarantine/stt_manager.py|stt_manager]]
    - ============================================================

### Imports from

- [[nova_backend/src/actions/action_request.py|action_request]] (`src.actions.action_request`)
- [[nova_backend/src/actions/action_types.py|action_types]] (`src.actions.action_types`)

## `audio` (1 files)

### Files

- [[nova_backend/src/audio/audio_task_runner.py|audio_task_runner]]
    - def run_speech_task(task: Callable[[], None]) -> None:

### Imported by

- [[nova_backend/src/voice/tts_engine.py|tts_engine]]

## `audit` (2 files)

_Runtime audit utilities for NovaLIS._

### Files

- [[nova_backend/src/audit/__init__.py|__init__ - src/audit]]
- [[nova_backend/src/audit/runtime_auditor.py|runtime_auditor]]
    - PROJECT_ROOT = Path(__file__).resolve().parents[3]

### Imports from

- [[nova_backend/src/build_phase.py|build_phase]] (`src.build_phase`)
- [[nova_backend/src/connectors/package_registry.py|package_registry]] (`src.connectors.package_registry`)
- [[nova_backend/src/governor/execute_boundary/execute_boundary.py|execute_boundary]] (`src.governor.execute_boundary.execute_boundary`)
- [[nova_backend/src/governor/governor_mediator.py|governor_mediator - src/governor]] (`src.governor.governor_mediator`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/tests/governance/test_mediator_registry_enforcement.py|test_mediator_registry_enforcement]]
- [[nova_backend/tests/governance/test_runtime_snapshot.py|test_runtime_snapshot]]
- [[nova_backend/tests/test_runtime_auditor.py|test_runtime_auditor]]
- [[nova_backend/tests/test_runtime_governance_docs.py|test_runtime_governance_docs]]
- [[scripts/generate_runtime_docs.py|generate_runtime_docs]]

### Tests

- [[nova_backend/tests/test_runtime_auditor.py|test_runtime_auditor]]

## `base_skill` (1 files)

### Files

- [[nova_backend/src/base_skill.py|base_skill]]
    - Base skill classes and result structure for NovaLIS.

### Imported by

- [[nova_backend/src/conversation/general_chat_runtime.py|general_chat_runtime]]
- [[nova_backend/src/skill_registry.py|skill_registry]]
- [[nova_backend/src/skills/calendar.py|calendar]]
- [[nova_backend/src/skills/executor_adapter.py|executor_adapter]]
- [[nova_backend/src/skills/general_chat.py|general_chat]]
- [[nova_backend/src/skills/news.py|news]]
- [[nova_backend/src/skills/system.py|system]]
- [[nova_backend/src/skills/weather.py|weather]]
- [[nova_backend/src/skills/web_search.py|web_search - src/skills]]
- [[nova_backend/tests/conversation/test_general_chat_runtime.py|test_general_chat_runtime]]
- [[nova_backend/tests/executors/test_info_snapshot_executor.py|test_info_snapshot_executor]]
- [[nova_backend/tests/phase45/test_brain_server_basic_conversation.py|test_brain_server_basic_conversation]]

## `brain_server` (1 files)

### Files

- [[nova_backend/src/brain_server.py|brain_server]]
    - NovaLIS Brain Server - Phase 4 Staging

### Imports from

- [[nova_backend/src/actions/action_result.py|action_result]] (`src.actions.action_result`)
- [[nova_backend/src/api/audit_api.py|audit_api]] (`src.api.audit_api`)
- [[nova_backend/src/api/bridge_api.py|bridge_api]] (`src.api.bridge_api`)
- [[nova_backend/src/api/connections_api.py|connections_api]] (`src.api.connections_api`)
- [[nova_backend/src/api/live_screen_api.py|live_screen_api]] (`src.api.live_screen_api`)
- [[nova_backend/src/api/memory_api.py|memory_api]] (`src.api.memory_api`)
- [[nova_backend/src/api/openclaw_agent_api.py|openclaw_agent_api]] (`src.api.openclaw_agent_api`)
- [[nova_backend/src/api/profile_api.py|profile_api]] (`src.api.profile_api`)
- [[nova_backend/src/api/settings_api.py|settings_api]] (`src.api.settings_api`)
- [[nova_backend/src/api/workspace_api.py|workspace_api]] (`src.api.workspace_api`)
- [[nova_backend/src/audit/runtime_auditor.py|runtime_auditor]] (`src.audit.runtime_auditor`)
- [[nova_backend/src/build_phase.py|build_phase]] (`src.build_phase`)
- [[nova_backend/src/connections/connections_store.py|connections_store]] (`src.connections.connections_store`)
- [[nova_backend/src/conversation/clarify_prompts.py|clarify_prompts]] (`src.conversation.clarify_prompts`)
- [[nova_backend/src/conversation/complexity_heuristics.py|complexity_heuristics]] (`src.conversation.complexity_heuristics`)
- [[nova_backend/src/conversation/general_chat_runtime.py|general_chat_runtime]] (`src.conversation.general_chat_runtime`)
- [[nova_backend/src/conversation/response_formatter.py|response_formatter]] (`src.conversation.response_formatter`)
- [[nova_backend/src/conversation/review_followthrough.py|review_followthrough]] (`src.conversation.review_followthrough`)
- [[nova_backend/src/conversation/session_router.py|session_router]] (`src.conversation.session_router`)
- [[nova_backend/src/conversation/thought_store.py|thought_store]] (`src.conversation.thought_store`)
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]] (`src.executors.os_diagnostics_executor`)
- [[nova_backend/src/governor/governor.py|governor]] (`src.governor.governor`)
- [[nova_backend/src/governor/governor_mediator.py|governor_mediator - src/governor]] (`src.governor.governor_mediator`)
- [[nova_backend/src/memory/governed_memory_store.py|governed_memory_store]] (`src.memory.governed_memory_store`)
- [[nova_backend/src/memory/quick_corrections.py|quick_corrections]] (`src.memory.quick_corrections`)
- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]] (`src.openclaw.agent_runner`)
- [[nova_backend/src/openclaw/agent_runtime_store.py|agent_runtime_store]] (`src.openclaw.agent_runtime_store`)
- [[nova_backend/src/openclaw/agent_scheduler.py|agent_scheduler]] (`src.openclaw.agent_scheduler`)
- [[nova_backend/src/patterns/pattern_review_store.py|pattern_review_store - src/patterns]] (`src.patterns.pattern_review_store`)
- [[nova_backend/src/personality/conversation_personality_agent.py|conversation_personality_agent]] (`src.personality.conversation_personality_agent`)
- [[nova_backend/src/personality/core.py|core]] (`src.personality.core`)
- [[nova_backend/src/personality/interface_agent.py|interface_agent]] (`src.personality.interface_agent`)
- [[nova_backend/src/personality/nova_style_contract.py|nova_style_contract]] (`src.personality.nova_style_contract`)
- [[nova_backend/src/personality/tone_profile_store.py|tone_profile_store]] (`src.personality.tone_profile_store`)
- [[nova_backend/src/policies/atomic_policy_store.py|atomic_policy_store]] (`src.policies.atomic_policy_store`)
- [[nova_backend/src/routers/stt.py|stt]] (`src.routers.stt`)
- [[nova_backend/src/settings/runtime_settings_store.py|runtime_settings_store]] (`src.settings.runtime_settings_store`)
- [[nova_backend/src/speech_state.py|speech_state]] (`src.speech_state`)
- [[nova_backend/src/tasks/notification_schedule_store.py|notification_schedule_store]] (`src.tasks.notification_schedule_store`)
- [[nova_backend/src/trust/failure_ladder.py|failure_ladder]] (`src.trust.failure_ladder`)
- [[nova_backend/src/trust/trust_contract.py|trust_contract]] (`src.trust.trust_contract`)
- [[nova_backend/src/usage/provider_usage_store.py|provider_usage_store]] (`src.usage.provider_usage_store`)
- [[nova_backend/src/utils/local_request_guard.py|local_request_guard]] (`src.utils.local_request_guard`)
- [[nova_backend/src/utils/path_resolver.py|path_resolver]] (`src.utils.path_resolver`)
- [[nova_backend/src/utils/web_target_planner.py|web_target_planner - src/utils]] (`src.utils.web_target_planner`)
- [[nova_backend/src/voice/stt_pipeline.py|stt_pipeline]] (`src.voice.stt_pipeline`)
- [[nova_backend/src/voice/tts_engine.py|tts_engine]] (`src.voice.tts_engine`)
- [[nova_backend/src/voice/voice_agent.py|voice_agent]] (`src.voice.voice_agent`)
- [[nova_backend/src/websocket/intent_patterns.py|intent_patterns]] (`src.websocket.intent_patterns`)
- [[nova_backend/src/websocket/session_handler.py|session_handler]] (`src.websocket.session_handler`)
- [[nova_backend/src/working_context/assistive_noticing.py|assistive_noticing]] (`src.working_context.assistive_noticing`)
- [[nova_backend/src/working_context/context_store.py|context_store]] (`src.working_context.context_store`)
- [[nova_backend/src/working_context/operational_remembrance.py|operational_remembrance]] (`src.working_context.operational_remembrance`)
- [[nova_backend/src/working_context/project_threads.py|project_threads]] (`src.working_context.project_threads`)

### Imported by

- [[nova_backend/tests/phase5/test_thread_change_summary.py|test_thread_change_summary]]

## `build_phase` (1 files)

### Files

- [[nova_backend/src/build_phase.py|build_phase]]
    - Compile-time style phase gate. This constant is intentionally static in source

### Imported by

- [[nova_backend/src/audit/runtime_auditor.py|runtime_auditor]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/governor/agent_orchestrator.py|agent_orchestrator - src/governor]]
- [[nova_backend/src/governor/policy_executor_gate.py|policy_executor_gate]]
- [[nova_backend/src/personality/core.py|core]]
- [[nova_backend/src/websocket/intent_patterns.py|intent_patterns]]
- [[nova_backend/tests/phase42/test_phase42_runtime_lock.py|test_phase42_runtime_lock]]
- [[nova_backend/tests/test_registry_phase_alignment.py|test_registry_phase_alignment]]

## `capabilities` (2 files)

### Files

- [[nova_backend/src/capabilities/__init__.py|__init__ - src/capabilities]]
- [[nova_backend/src/capabilities/capabilities.py|capabilities]]
    - NovaLIS capability declarations.

## `cognition` (8 files)

_CognitiveModule,_

### Files

- [[nova_backend/src/cognition/__init__.py|__init__ - src/cognition]]
- [[nova_backend/src/cognition/cognitive_layer_contract.py|cognitive_layer_contract]]
    - class CognitiveMode(str, Enum):
- [[nova_backend/src/cognition/cognitive_operation_logger.py|cognitive_operation_logger]]
    - logger = logging.getLogger(__name__)
- [[nova_backend/src/cognition/intelligence_report_contract.py|intelligence_report_contract]]
    - REQUIRED_STRUCTURED_FIELDS = (
- [[nova_backend/src/cognition/modules/__init__.py|__init__ - cognition/modules]]
- [[nova_backend/src/cognition/modules/reporting_module.py|reporting_module]]
    - CognitiveModule,
- [[nova_backend/src/cognition/modules/research_module.py|research_module]]
    - CognitiveModule,
- [[nova_backend/src/cognition/modules/verification_module.py|verification_module]]
    - CognitiveModule,

### Imports from

- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)

### Imported by

- [[nova_backend/src/conversation/deepseek_bridge.py|deepseek_bridge]]
- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]]
- [[nova_backend/tests/governance/test_cognition_module_scaffolds.py|test_cognition_module_scaffolds]]
- [[nova_backend/tests/governance/test_cognitive_layer_contract.py|test_cognitive_layer_contract]]
- [[nova_backend/tests/phase42/test_intelligence_report_contract.py|test_intelligence_report_contract]]

### Tests

- [[nova_backend/tests/governance/test_cognitive_layer_contract.py|test_cognitive_layer_contract]]
- [[nova_backend/tests/phase42/test_intelligence_report_contract.py|test_intelligence_report_contract]]

## `connections` (2 files)

### Files

- [[nova_backend/src/connections/__init__.py|__init__ - src/connections]]
- [[nova_backend/src/connections/connections_store.py|connections_store]]
    - ConnectionsStore — persists provider keys across restarts.

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/api/connections_api.py|connections_api]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/identity/nova_self_awareness.py|nova_self_awareness]]
- [[nova_backend/tests/phase45/test_connections_api.py|test_connections_api]]
- [[nova_backend/tests/phase45/test_connections_store.py|test_connections_store]]

### Tests

- [[nova_backend/tests/phase45/test_connections_store.py|test_connections_store]]

## `connectors` (3 files)

___all__ = ["ConnectorPackage", "ConnectorPackageRegistry"]_

### Files

- [[nova_backend/src/connectors/__init__.py|__init__ - src/connectors]]
- [[nova_backend/src/connectors/email_connector.py|email_connector]]
    - Email Connector — interface stub for inbox_check (Phase 8+).
- [[nova_backend/src/connectors/package_registry.py|package_registry]]
    - CONNECTOR_PACKAGES_PATH = Path(__file__).resolve().parents[1] / "config" / "connector_packages.json

### Imports from

- [[nova_backend/src/governor/exceptions.py|exceptions]] (`src.governor.exceptions`)

### Imported by

- [[nova_backend/src/audit/runtime_auditor.py|runtime_auditor]]
- [[nova_backend/tests/test_connector_package_registry.py|test_connector_package_registry]]

## `context` (5 files)

___all__ = ["ContextSnapshotService"]_

### Files

- [[nova_backend/src/context/__init__.py|__init__ - src/context]]
- [[nova_backend/src/context/active_window.py|active_window]]
    - def get_active_window() -> dict[str, Any]:
- [[nova_backend/src/context/browser_context.py|browser_context]]
    - _BROWSER_HINTS = ("chrome", "firefox", "edge", "safari", "brave", "opera")
- [[nova_backend/src/context/context_snapshot_service.py|context_snapshot_service]]
    - ActiveWindowProvider = Callable[[], dict[str, Any]]
- [[nova_backend/src/context/system_context.py|system_context]]
    - def get_system_context() -> dict[str, str]:

### Imports from

- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/perception/cursor_locator.py|cursor_locator]] (`src.perception.cursor_locator`)

### Imported by

- [[nova_backend/src/executors/explain_anything_executor.py|explain_anything_executor]]
- [[nova_backend/src/executors/screen_capture_executor.py|screen_capture_executor]]
- [[nova_backend/tests/phase45/test_context_snapshot_contract.py|test_context_snapshot_contract]]

## `conversation` (16 files)

_Conversation-layer helpers for cognitive escalation (Phase-4.2 staging)._

### Files

- [[nova_backend/src/conversation/__init__.py|__init__ - src/conversation]]
- [[nova_backend/src/conversation/clarify_prompts.py|clarify_prompts]]
    - Deterministic clarification prompt bank for conversational UX polish.
- [[nova_backend/src/conversation/complexity_heuristics.py|complexity_heuristics]]
    - class ComplexityHeuristics:
- [[nova_backend/src/conversation/conversation_decision.py|conversation_decision]]
    - class ConversationMode(str, Enum):
- [[nova_backend/src/conversation/conversation_router.py|conversation_router]]
    - class ConversationRouter:
- [[nova_backend/src/conversation/deepseek_bridge.py|deepseek_bridge]]
    - logger = logging.getLogger(__name__)
- [[nova_backend/src/conversation/deepseek_safety_wrapper.py|deepseek_safety_wrapper]]
    - class DeepSeekSafetyWrapper:
- [[nova_backend/src/conversation/escalation_policy.py|escalation_policy]]
    - PolicyDecision = Literal["ALLOW_ANALYSIS_ONLY", "DENY", "ASK_USER"]
- [[nova_backend/src/conversation/general_chat_runtime.py|general_chat_runtime]]
    - Helpers for keeping general-chat fallback behavior out of the websocket monolith.
- [[nova_backend/src/conversation/prompts.py|prompts]]
    - SYSTEM_PROMPT = (
- [[nova_backend/src/conversation/response_formatter.py|response_formatter]]
    - class ResponseFormatter:
- [[nova_backend/src/conversation/response_style_router.py|response_style_router]]
    - class ResponseStyle(str, Enum):
- [[nova_backend/src/conversation/review_followthrough.py|review_followthrough]]
    - def build_review_followthrough_snapshot(
- [[nova_backend/src/conversation/safety_filter.py|safety_filter]]
    - class SafetyFilter:
- [[nova_backend/src/conversation/session_router.py|session_router]]
    - WEB_OPEN_CONFIRM_YES = {
- [[nova_backend/src/conversation/thought_store.py|thought_store]]
    - class ThoughtStore:

### Imports from

- [[nova_backend/src/base_skill.py|base_skill]] (`src.base_skill`)
- [[nova_backend/src/cognition/cognitive_operation_logger.py|cognitive_operation_logger]] (`src.cognition.cognitive_operation_logger`)
- [[nova_backend/src/governor/network_mediator.py|network_mediator]] (`src.governor.network_mediator`)
- [[nova_backend/src/llm/llm_gateway.py|llm_gateway]] (`src.llm.llm_gateway`)
- [[nova_backend/src/personality/nova_style_contract.py|nova_style_contract]] (`src.personality.nova_style_contract`)
- [[nova_backend/src/personality/tone_profile_store.py|tone_profile_store]] (`src.personality.tone_profile_store`)
- [[nova_backend/src/skills/general_chat.py|general_chat]] (`src.skills.general_chat`)
- [[nova_backend/src/usage/provider_usage_store.py|provider_usage_store]] (`src.usage.provider_usage_store`)
- [[nova_backend/src/working_context/project_threads.py|project_threads]] (`src.working_context.project_threads`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/analysis_document_executor.py|analysis_document_executor]]
- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]]
- [[nova_backend/src/executors/response_verification_executor.py|response_verification_executor]]
- [[nova_backend/src/skills/general_chat.py|general_chat]]
- [[nova_backend/src/voice/tts_engine.py|tts_engine]]
- [[nova_backend/src/voice/voice_agent.py|voice_agent]]
- [[nova_backend/tests/conversation/test_clarify_prompts.py|test_clarify_prompts]]
- [[nova_backend/tests/conversation/test_complexity_heuristics.py|test_complexity_heuristics]]
- [[nova_backend/tests/conversation/test_deepseek_bridge.py|test_deepseek_bridge]]
- [[nova_backend/tests/conversation/test_deepseek_safety_wrapper.py|test_deepseek_safety_wrapper]]
- [[nova_backend/tests/conversation/test_deepseek_usage_visibility.py|test_deepseek_usage_visibility]]
- [[nova_backend/tests/conversation/test_escalation_policy.py|test_escalation_policy]]
- [[nova_backend/tests/conversation/test_general_chat_runtime.py|test_general_chat_runtime]]
- [[nova_backend/tests/conversation/test_response_formatter.py|test_response_formatter]]
- _…and 13 more_

### Tests

- [[nova_backend/tests/conversation/test_clarify_prompts.py|test_clarify_prompts]]
- [[nova_backend/tests/conversation/test_complexity_heuristics.py|test_complexity_heuristics]]
- [[nova_backend/tests/conversation/test_conversation_router.py|test_conversation_router]]
- [[nova_backend/tests/conversation/test_deepseek_bridge.py|test_deepseek_bridge]]
- [[nova_backend/tests/conversation/test_deepseek_safety_wrapper.py|test_deepseek_safety_wrapper]]
- [[nova_backend/tests/conversation/test_escalation_policy.py|test_escalation_policy]]
- [[nova_backend/tests/conversation/test_general_chat_runtime.py|test_general_chat_runtime]]
- [[nova_backend/tests/conversation/test_response_formatter.py|test_response_formatter]]
- [[nova_backend/tests/conversation/test_response_style_router.py|test_response_style_router]]
- [[nova_backend/tests/conversation/test_safety_filter.py|test_safety_filter]]
- [[nova_backend/tests/conversation/test_session_router.py|test_session_router]]
- [[nova_backend/tests/conversation/test_thought_store.py|test_thought_store]]

## `debug` (2 files)

_Debug utilities package._

### Files

- [[nova_backend/src/debug/__init__.py|__init__ - src/debug]]
- [[nova_backend/src/debug/cognitive_trace.py|cognitive_trace]]
    - @dataclass

### Imported by

- [[nova_backend/tests/simulation/conversation_simulator.py|conversation_simulator]]

## `executors` (21 files)

### Files

- [[nova_backend/src/executors/__init__.py|__init__ - src/executors]]
- [[nova_backend/src/executors/analysis_document_executor.py|analysis_document_executor]]
    - class AnalysisDocumentExecutor:
- [[nova_backend/src/executors/brightness_executor.py|brightness_executor]]
    - class BrightnessExecutor:
- [[nova_backend/src/executors/explain_anything_executor.py|explain_anything_executor]]
    - class ExplainAnythingExecutor:
- [[nova_backend/src/executors/external_reasoning_executor.py|external_reasoning_executor]]
    - class ExternalReasoningExecutor:
- [[nova_backend/src/executors/info_snapshot_executor.py|info_snapshot_executor]]
    - def _run_skill(skill: Any, query: str) -> Any:
- [[nova_backend/src/executors/media_executor.py|media_executor]]
    - class MediaExecutor:
- [[nova_backend/src/executors/memory_governance_executor.py|memory_governance_executor]]
    - class MemoryGovernanceExecutor:
- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]]
    - validate_rendered_report_text,
- [[nova_backend/src/executors/news_intelligence_executor.py|news_intelligence_executor]]
    - MAX_HEADLINES_PER_SUMMARY = 3
- [[nova_backend/src/executors/open_folder_executor.py|open_folder_executor]]
    - PRESET_FOLDERS = {
- [[nova_backend/src/executors/openclaw_execute_executor.py|openclaw_execute_executor]]
    - OpenClaw Execute Executor — cap 63 (openclaw_execute)
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
    - class OSDiagnosticsExecutor:
- [[nova_backend/src/executors/response_verification_executor.py|response_verification_executor]]
    - class ResponseVerificationExecutor:
- [[nova_backend/src/executors/screen_analysis_executor.py|screen_analysis_executor]]
    - class ScreenAnalysisExecutor:
- [[nova_backend/src/executors/screen_capture_executor.py|screen_capture_executor]]
    - class ScreenCaptureExecutor:
- [[nova_backend/src/executors/story_tracker_executor.py|story_tracker_executor]]
    - PROJECT_ROOT = Path(__file__).resolve().parents[3]
- [[nova_backend/src/executors/tts_executor.py|tts_executor]]
    - log = logging.getLogger("nova")
- [[nova_backend/src/executors/volume_executor.py|volume_executor]]
    - class VolumeExecutor:
- [[nova_backend/src/executors/web_search_executor.py|web_search_executor]]
    - logger = logging.getLogger(__name__)
- [[nova_backend/src/executors/webpage_launch_executor.py|webpage_launch_executor]]
    - class WebpageLaunchExecutor:

### Imports from

- [[nova_backend/src/actions/action_result.py|action_result]] (`src.actions.action_result`)
- [[nova_backend/src/build_phase.py|build_phase]] (`src.build_phase`)
- [[nova_backend/src/cognition/cognitive_layer_contract.py|cognitive_layer_contract]] (`src.cognition.cognitive_layer_contract`)
- [[nova_backend/src/cognition/intelligence_report_contract.py|intelligence_report_contract]] (`src.cognition.intelligence_report_contract`)
- [[nova_backend/src/cognition/modules/__init__.py|__init__ - cognition/modules]] (`src.cognition.modules`)
- [[nova_backend/src/context/context_snapshot_service.py|context_snapshot_service]] (`src.context.context_snapshot_service`)
- [[nova_backend/src/conversation/deepseek_bridge.py|deepseek_bridge]] (`src.conversation.deepseek_bridge`)
- [[nova_backend/src/conversation/deepseek_safety_wrapper.py|deepseek_safety_wrapper]] (`src.conversation.deepseek_safety_wrapper`)
- [[nova_backend/src/governor/capability_registry.py|capability_registry]] (`src.governor.capability_registry`)
- [[nova_backend/src/governor/capability_topology.py|capability_topology]] (`src.governor.capability_topology`)
- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/llm/llm_gateway.py|llm_gateway]] (`src.llm.llm_gateway`)
- [[nova_backend/src/llm/llm_manager.py|llm_manager]] (`src.llm.llm_manager`)
- [[nova_backend/src/memory/governed_memory_store.py|governed_memory_store]] (`src.memory.governed_memory_store`)
- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]] (`src.openclaw.agent_runner`)
- [[nova_backend/src/openclaw/agent_runtime_store.py|agent_runtime_store]] (`src.openclaw.agent_runtime_store`)
- [[nova_backend/src/perception/cursor_locator.py|cursor_locator]] (`src.perception.cursor_locator`)
- [[nova_backend/src/perception/explain_anything.py|explain_anything]] (`src.perception.explain_anything`)
- [[nova_backend/src/perception/ocr_pipeline.py|ocr_pipeline]] (`src.perception.ocr_pipeline`)
- [[nova_backend/src/perception/screen_capture.py|screen_capture]] (`src.perception.screen_capture`)
- [[nova_backend/src/perception/vision_analyzer.py|vision_analyzer]] (`src.perception.vision_analyzer`)
- [[nova_backend/src/personality/tone_profile_store.py|tone_profile_store]] (`src.personality.tone_profile_store`)
- [[nova_backend/src/policies/atomic_policy_store.py|atomic_policy_store]] (`src.policies.atomic_policy_store`)
- [[nova_backend/src/rendering/intelligence_brief_renderer.py|intelligence_brief_renderer]] (`src.rendering.intelligence_brief_renderer`)
- [[nova_backend/src/rendering/speech_formatter.py|speech_formatter]] (`src.rendering.speech_formatter`)
- [[nova_backend/src/settings/runtime_settings_store.py|runtime_settings_store]] (`src.settings.runtime_settings_store`)
- [[nova_backend/src/skills/calendar.py|calendar]] (`src.skills.calendar`)
- [[nova_backend/src/skills/news.py|news]] (`src.skills.news`)
- [[nova_backend/src/skills/weather.py|weather]] (`src.skills.weather`)
- [[nova_backend/src/speech_state.py|speech_state]] (`src.speech_state`)
- [[nova_backend/src/system_control/system_control_executor.py|system_control_executor]] (`src.system_control.system_control_executor`)
- [[nova_backend/src/tasks/notification_schedule_store.py|notification_schedule_store]] (`src.tasks.notification_schedule_store`)
- [[nova_backend/src/usage/provider_usage_store.py|provider_usage_store]] (`src.usage.provider_usage_store`)
- [[nova_backend/src/utils/content_extractor.py|content_extractor]] (`src.utils.content_extractor`)
- [[nova_backend/src/utils/web_target_planner.py|web_target_planner - src/utils]] (`src.utils.web_target_planner`)
- [[nova_backend/src/validation/pipeline.py|pipeline]] (`src.validation.pipeline`)
- [[nova_backend/src/voice/tts_engine.py|tts_engine]] (`src.voice.tts_engine`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/governor/governor.py|governor]]
- [[nova_backend/src/openclaw/tool_registry.py|tool_registry]]
- [[nova_backend/src/skills/web_search.py|web_search - src/skills]]
- [[nova_backend/src/voice/tts_engine.py|tts_engine]]
- [[nova_backend/tests/adversarial/test_search_injection_no_escalation.py|test_search_injection_no_escalation]]
- [[nova_backend/tests/executors/test_analysis_document_executor.py|test_analysis_document_executor]]
- [[nova_backend/tests/executors/test_explain_anything_executor.py|test_explain_anything_executor]]
- [[nova_backend/tests/executors/test_external_reasoning_executor.py|test_external_reasoning_executor]]
- [[nova_backend/tests/executors/test_info_snapshot_executor.py|test_info_snapshot_executor]]
- [[nova_backend/tests/executors/test_local_action_executors.py|test_local_action_executors]]
- [[nova_backend/tests/executors/test_local_control_executors.py|test_local_control_executors]]
- [[nova_backend/tests/executors/test_multi_source_reporting_executor.py|test_multi_source_reporting_executor]]
- [[nova_backend/tests/executors/test_news_intelligence_executor.py|test_news_intelligence_executor]]
- [[nova_backend/tests/executors/test_open_folder_executor.py|test_open_folder_executor - tests/executors]]
- _…and 18 more_

### Tests

- [[nova_backend/tests/executors/test_analysis_document_executor.py|test_analysis_document_executor]]
- [[nova_backend/tests/executors/test_explain_anything_executor.py|test_explain_anything_executor]]
- [[nova_backend/tests/executors/test_external_reasoning_executor.py|test_external_reasoning_executor]]
- [[nova_backend/tests/executors/test_info_snapshot_executor.py|test_info_snapshot_executor]]
- [[nova_backend/tests/executors/test_multi_source_reporting_executor.py|test_multi_source_reporting_executor]]
- [[nova_backend/tests/executors/test_news_intelligence_executor.py|test_news_intelligence_executor]]
- [[nova_backend/tests/executors/test_open_folder_executor.py|test_open_folder_executor - tests/executors]]
- [[nova_backend/tests/executors/test_response_verification_executor.py|test_response_verification_executor]]
- [[nova_backend/tests/executors/test_screen_analysis_executor.py|test_screen_analysis_executor]]
- [[nova_backend/tests/executors/test_story_tracker_executor.py|test_story_tracker_executor]]
- [[nova_backend/tests/executors/test_tts_executor.py|test_tts_executor]]
- [[nova_backend/tests/executors/test_web_search_executor.py|test_web_search_executor]]
- [[nova_backend/tests/executors/test_webpage_launch_executor.py|test_webpage_launch_executor]]
- [[nova_backend/tests/openclaw/test_openclaw_execute_executor.py|test_openclaw_execute_executor]]
- [[nova_backend/tests/phase45/test_screen_capture_executor.py|test_screen_capture_executor]]
- [[nova_backend/tests/phase5/test_memory_governance_executor.py|test_memory_governance_executor]]
- [[nova_backend/tests/test_open_folder_executor.py|test_open_folder_executor - nova_backend/tests]]

## `gates` (2 files)

### Files

- [[nova_backend/src/gates/__init__.py|__init__ - src/gates]]
- [[nova_backend/src/gates/confirmation_gate.py|confirmation_gate]]
    - ConfirmationGate — Provably Passive

### Imported by

- [[nova_backend/tests/test_confirmation_gate.py|test_confirmation_gate]]

### Tests

- [[nova_backend/tests/test_confirmation_gate.py|test_confirmation_gate]]

## `governor` (12 files)

_ExecuteBoundary,_

### Files

- [[nova_backend/src/governor/agent_orchestrator.py|agent_orchestrator - src/governor]]
    - @dataclass(frozen=True)
- [[nova_backend/src/governor/capability_registry.py|capability_registry]]
    - REGISTRY_PATH = Path(__file__).resolve().parents[1] / "config" / "registry.json
- [[nova_backend/src/governor/capability_topology.py|capability_topology]]
    - AUTHORITY_CLASS_ORDER = {
- [[nova_backend/src/governor/exceptions.py|exceptions]]
    - class CapabilityRegistryError(Exception):
- [[nova_backend/src/governor/execute_boundary/__init__.py|__init__ - governor/execute_boundary]]
- [[nova_backend/src/governor/execute_boundary/execute_boundary.py|execute_boundary]]
    - ExecuteBoundary – Governor‑owned execution gate.
- [[nova_backend/src/governor/governor.py|governor]]
    - Governor - constitutional authority spine.
- [[nova_backend/src/governor/governor_mediator.py|governor_mediator - src/governor]]
    - PENDING_CLARIFICATION_TTL_SECONDS = 60.0
- [[nova_backend/src/governor/network_mediator.py|network_mediator]]
    - NETWORK_TIMEOUT = 5  # seconds (default)
- [[nova_backend/src/governor/policy_executor_gate.py|policy_executor_gate]]
    - def _bool_label(value: bool) -> str:
- [[nova_backend/src/governor/single_action_queue.py|single_action_queue]]
    - class SingleActionQueue:
- [[nova_backend/src/governor/web_target_planner.py|web_target_planner - src/governor]]
    - Backward-compatible re-export for older imports.

### Imports from

- [[nova_backend/src/actions/action_request.py|action_request]] (`src.actions.action_request`)
- [[nova_backend/src/actions/action_result.py|action_result]] (`src.actions.action_result`)
- [[nova_backend/src/agents/base.py|base]] (`src.agents.base`)
- [[nova_backend/src/agents/context.py|context]] (`src.agents.context`)
- [[nova_backend/src/build_phase.py|build_phase]] (`src.build_phase`)
- [[nova_backend/src/executors/analysis_document_executor.py|analysis_document_executor]] (`src.executors.analysis_document_executor`)
- [[nova_backend/src/executors/brightness_executor.py|brightness_executor]] (`src.executors.brightness_executor`)
- [[nova_backend/src/executors/explain_anything_executor.py|explain_anything_executor]] (`src.executors.explain_anything_executor`)
- [[nova_backend/src/executors/external_reasoning_executor.py|external_reasoning_executor]] (`src.executors.external_reasoning_executor`)
- [[nova_backend/src/executors/info_snapshot_executor.py|info_snapshot_executor]] (`src.executors.info_snapshot_executor`)
- [[nova_backend/src/executors/media_executor.py|media_executor]] (`src.executors.media_executor`)
- [[nova_backend/src/executors/memory_governance_executor.py|memory_governance_executor]] (`src.executors.memory_governance_executor`)
- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]] (`src.executors.multi_source_reporting_executor`)
- [[nova_backend/src/executors/news_intelligence_executor.py|news_intelligence_executor]] (`src.executors.news_intelligence_executor`)
- [[nova_backend/src/executors/open_folder_executor.py|open_folder_executor]] (`src.executors.open_folder_executor`)
- [[nova_backend/src/executors/openclaw_execute_executor.py|openclaw_execute_executor]] (`src.executors.openclaw_execute_executor`)
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]] (`src.executors.os_diagnostics_executor`)
- [[nova_backend/src/executors/response_verification_executor.py|response_verification_executor]] (`src.executors.response_verification_executor`)
- [[nova_backend/src/executors/screen_analysis_executor.py|screen_analysis_executor]] (`src.executors.screen_analysis_executor`)
- [[nova_backend/src/executors/screen_capture_executor.py|screen_capture_executor]] (`src.executors.screen_capture_executor`)
- [[nova_backend/src/executors/story_tracker_executor.py|story_tracker_executor]] (`src.executors.story_tracker_executor`)
- [[nova_backend/src/executors/tts_executor.py|tts_executor]] (`src.executors.tts_executor`)
- [[nova_backend/src/executors/volume_executor.py|volume_executor]] (`src.executors.volume_executor`)
- [[nova_backend/src/executors/web_search_executor.py|web_search_executor]] (`src.executors.web_search_executor`)
- [[nova_backend/src/executors/webpage_launch_executor.py|webpage_launch_executor]] (`src.executors.webpage_launch_executor`)
- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/policies/policy_validator.py|policy_validator]] (`src.policies.policy_validator`)
- [[nova_backend/src/usage/provider_usage_store.py|provider_usage_store]] (`src.usage.provider_usage_store`)
- [[nova_backend/src/utils/web_target_planner.py|web_target_planner - src/utils]] (`src.utils.web_target_planner`)

### Imported by

- [[nova_backend/src/api/audit_api.py|audit_api]]
- [[nova_backend/src/api/bridge_api.py|bridge_api]]
- [[nova_backend/src/audit/runtime_auditor.py|runtime_auditor]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/connectors/package_registry.py|package_registry]]
- [[nova_backend/src/conversation/general_chat_runtime.py|general_chat_runtime]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/identity/nova_self_awareness.py|nova_self_awareness]]
- [[nova_backend/src/ledger/writer.py|writer]]
- [[nova_backend/src/llm/llm_manager.py|llm_manager]]
- [[nova_backend/src/llm/llm_manager_vlock.py|llm_manager_vlock]]
- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]]
- [[nova_backend/src/personality/core.py|core]]
- [[nova_backend/src/providers/openai_responses_lane.py|openai_responses_lane]]
- [[nova_backend/src/services/weather_service.py|weather_service]]
- _…and 45 more_

### Tests

- [[nova_backend/tests/phase6/test_capability_topology.py|test_capability_topology]]
- [[nova_backend/tests/phase6/test_policy_executor_gate.py|test_policy_executor_gate]]
- [[nova_backend/tests/test_single_action_queue.py|test_single_action_queue]]

## `identity` (2 files)

### Files

- [[nova_backend/src/identity/__init__.py|__init__ - src/identity]]
- [[nova_backend/src/identity/nova_self_awareness.py|nova_self_awareness]]
    - Nova self-awareness context builder.

### Imports from

- [[nova_backend/src/connections/connections_store.py|connections_store]] (`src.connections.connections_store`)
- [[nova_backend/src/governor/capability_registry.py|capability_registry]] (`src.governor.capability_registry`)
- [[nova_backend/src/llm/llm_manager.py|llm_manager]] (`src.llm.llm_manager`)
- [[nova_backend/src/openclaw/tool_registry.py|tool_registry]] (`src.openclaw.tool_registry`)
- [[nova_backend/src/settings/runtime_settings_store.py|runtime_settings_store]] (`src.settings.runtime_settings_store`)

### Imported by

- [[nova_backend/src/skills/general_chat.py|general_chat]]
- [[nova_backend/tests/identity/test_nova_self_awareness.py|test_nova_self_awareness]]

### Tests

- [[nova_backend/tests/identity/test_nova_self_awareness.py|test_nova_self_awareness]]

## `ledger` (3 files)

### Files

- [[nova_backend/src/ledger/event_types.py|event_types]]
    - Canonical ledger event taxonomy for Phase-4 runtime.
- [[nova_backend/src/ledger/reader.py|reader]]
    - class LedgerAnalyzer:
- [[nova_backend/src/ledger/writer.py|writer]]
    - LEDGER_PATH = Path(__file__).resolve().parents[1] / "data" / "ledger.jsonl

### Imports from

- [[nova_backend/src/governor/exceptions.py|exceptions]] (`src.governor.exceptions`)

### Imported by

- [[nova_backend/src/api/live_screen_api.py|live_screen_api]]
- [[nova_backend/src/cognition/cognitive_operation_logger.py|cognitive_operation_logger]]
- [[nova_backend/src/context/context_snapshot_service.py|context_snapshot_service]]
- [[nova_backend/src/executors/explain_anything_executor.py|explain_anything_executor]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/executors/screen_analysis_executor.py|screen_analysis_executor]]
- [[nova_backend/src/executors/screen_capture_executor.py|screen_capture_executor]]
- [[nova_backend/src/executors/tts_executor.py|tts_executor]]
- [[nova_backend/src/executors/webpage_launch_executor.py|webpage_launch_executor]]
- [[nova_backend/src/governor/capability_registry.py|capability_registry]]
- [[nova_backend/src/governor/governor.py|governor]]
- [[nova_backend/src/governor/network_mediator.py|network_mediator]]
- [[nova_backend/src/llm/llm_manager.py|llm_manager]]
- [[nova_backend/src/llm/llm_manager_vlock.py|llm_manager_vlock]]
- [[nova_backend/src/llm/model_network_mediator.py|model_network_mediator]]
- _…and 10 more_

## `llm` (6 files)

### Files

- [[nova_backend/src/llm/inference_wrapper.py|inference_wrapper]]
    - This file contains the actual low‑level inference call.
- [[nova_backend/src/llm/llm_gateway.py|llm_gateway]]
    - logger = logging.getLogger(__name__)
- [[nova_backend/src/llm/llm_manager.py|llm_manager]]
    - logger = logging.getLogger(__name__)
- [[nova_backend/src/llm/llm_manager_vlock.py|llm_manager_vlock]]
    - Ollama LLM integration with constitutional version locking.
- [[nova_backend/src/llm/model_network_mediator.py|model_network_mediator]]
    - ALLOWED_SCHEMES = {"http", "https"}
- [[nova_backend/src/llm/system_prompt.py|system_prompt]]
    - SYSTEM_PROMPT = """You are Nova, a personal AI assistant who is genuinely helpful and warmly competent.

### Imports from

- [[nova_backend/src/governor/exceptions.py|exceptions]] (`src.governor.exceptions`)
- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/nova_config.py|nova_config]] (`src.nova_config`)

### Imported by

- [[nova_backend/src/agents/_llm_agent.py|_llm_agent]]
- [[nova_backend/src/api/settings_api.py|settings_api]]
- [[nova_backend/src/conversation/review_followthrough.py|review_followthrough]]
- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]]
- [[nova_backend/src/executors/news_intelligence_executor.py|news_intelligence_executor]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/executors/web_search_executor.py|web_search_executor]]
- [[nova_backend/src/identity/nova_self_awareness.py|nova_self_awareness]]
- [[nova_backend/src/nova_config.py|nova_config]]
- [[nova_backend/src/skills/general_chat.py|general_chat]]
- [[nova_backend/src/websocket/session_handler.py|session_handler]]
- [[nova_backend/tests/executors/test_local_control_executors.py|test_local_control_executors]]
- [[nova_backend/tests/governance/test_model_network_mediator_thread_safety.py|test_model_network_mediator_thread_safety]]
- [[nova_backend/tests/test_llm_manager_version_lock.py|test_llm_manager_version_lock]]
- [[nova_backend/tests/test_network_model_speech_correlation.py|test_network_model_speech_correlation]]

## `memory` (4 files)

### Files

- [[nova_backend/src/memory/governed_memory_store.py|governed_memory_store]]
    - _MEMORY_SEARCH_STOPWORDS = {
- [[nova_backend/src/memory/nova_self_memory_store.py|nova_self_memory_store]]
    - Nova self-memory store — Nova's own memory about the relationship and patterns.
- [[nova_backend/src/memory/quick_corrections.py|quick_corrections]]
    - Phase-3.5 Staged Governed Memory — Quick Corrections
- [[nova_backend/src/memory/user_memory_store.py|user_memory_store]]
    - Persistent user memory store — preferences, personal details, and observed patterns.

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/api/memory_api.py|memory_api]]
- [[nova_backend/src/api/profile_api.py|profile_api]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/memory_governance_executor.py|memory_governance_executor]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/skills/general_chat.py|general_chat]]
- [[nova_backend/src/websocket/session_handler.py|session_handler]]
- [[nova_backend/tests/phase45/test_profile_api.py|test_profile_api]]
- [[nova_backend/tests/phase5/test_memory_governance_executor.py|test_memory_governance_executor]]
- [[nova_backend/tests/test_governed_memory_store.py|test_governed_memory_store]]
- [[nova_backend/tests/test_memory_api.py|test_memory_api]]

### Tests

- [[nova_backend/tests/test_governed_memory_store.py|test_governed_memory_store]]

## `nova_config` (1 files)

### Files

- [[nova_backend/src/nova_config.py|nova_config]]
    - NovaLIS Core Configuration (Phase 1)

### Imports from

- [[nova_backend/src/llm/system_prompt.py|system_prompt]] (`src.llm.system_prompt`)

### Imported by

- [[nova_backend/src/llm/llm_manager.py|llm_manager]]
- [[nova_backend/src/llm/llm_manager_vlock.py|llm_manager_vlock]]

## `nova_protocol` (1 files)

### Files

- [[nova_backend/src/nova_protocol.py|nova_protocol]]
    - Nova Message Protocol v1 (NMPv1)

## `openclaw` (23 files)

_Governed OpenClaw foundations that remain inside Nova's runtime boundaries._

### Files

- [[nova_backend/src/openclaw/__init__.py|__init__ - src/openclaw]]
- [[nova_backend/src/openclaw/agent_execution_memory.py|agent_execution_memory]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_fallback_strategies.py|agent_fallback_strategies]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_orchestrator.py|agent_orchestrator - src/openclaw]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_parameter_templates.py|agent_parameter_templates]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_personality_bridge.py|agent_personality_bridge]]
    - DEFAULT_DELIVERY_MODE_BY_TEMPLATE = {
- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]]
    - OpenClawAgentPersonalityBridge,
- [[nova_backend/src/openclaw/agent_runtime_store.py|agent_runtime_store]]
    - DEFAULT_DELIVERY_MODE_BY_TEMPLATE,
- [[nova_backend/src/openclaw/agent_scheduler.py|agent_scheduler]]
    - logger = logging.getLogger(__name__)
- [[nova_backend/src/openclaw/agent_state_store.py|agent_state_store]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_thinking_loop.py|agent_thinking_loop]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_tool_chaining.py|agent_tool_chaining]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_tool_executor.py|agent_tool_executor]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/agent_tool_registry_bootstrap.py|agent_tool_registry_bootstrap]]
    - Legacy placeholder kept to document a retired module name.
- [[nova_backend/src/openclaw/execution_memory.py|execution_memory]]
    - Execution memory for OpenClaw — learn from past runs.
- [[nova_backend/src/openclaw/per_tool_budget.py|per_tool_budget]]
    - Per-tool budget tracking for OpenClaw.
- [[nova_backend/src/openclaw/robust_executor.py|robust_executor]]
    - Robust tool execution for OpenClaw — retry, fallback, parallel, and metering.
- [[nova_backend/src/openclaw/strict_preflight.py|strict_preflight]]
    - STRICT_FOUNDATION_LABEL = "Manual preflight active
- [[nova_backend/src/openclaw/task_envelope.py|task_envelope]]
    - def _utc_now_iso() -> str:
- [[nova_backend/src/openclaw/thinking_loop.py|thinking_loop]]
    - Multi-step agent thinking loop for OpenClaw.
- [[nova_backend/src/openclaw/tool_chain.py|tool_chain]]
    - Tool chaining and composition for OpenClaw.
- [[nova_backend/src/openclaw/tool_registry.py|tool_registry]]
    - Dynamic tool registry for OpenClaw agent intelligence layer.
- [[nova_backend/src/openclaw/user_tool_permissions.py|user_tool_permissions]]
    - Per-user tool permission layer for OpenClaw.

### Imports from

- [[nova_backend/src/executors/brightness_executor.py|brightness_executor]] (`src.executors.brightness_executor`)
- [[nova_backend/src/executors/media_executor.py|media_executor]] (`src.executors.media_executor`)
- [[nova_backend/src/executors/screen_capture_executor.py|screen_capture_executor]] (`src.executors.screen_capture_executor`)
- [[nova_backend/src/executors/volume_executor.py|volume_executor]] (`src.executors.volume_executor`)
- [[nova_backend/src/executors/webpage_launch_executor.py|webpage_launch_executor]] (`src.executors.webpage_launch_executor`)
- [[nova_backend/src/governor/network_mediator.py|network_mediator]] (`src.governor.network_mediator`)
- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/personality/conversation_personality_agent.py|conversation_personality_agent]] (`src.personality.conversation_personality_agent`)
- [[nova_backend/src/providers/openai_responses_lane.py|openai_responses_lane]] (`src.providers.openai_responses_lane`)
- [[nova_backend/src/settings/runtime_settings_store.py|runtime_settings_store]] (`src.settings.runtime_settings_store`)
- [[nova_backend/src/skills/calendar.py|calendar]] (`src.skills.calendar`)
- [[nova_backend/src/skills/executor_adapter.py|executor_adapter]] (`src.skills.executor_adapter`)
- [[nova_backend/src/skills/news.py|news]] (`src.skills.news`)
- [[nova_backend/src/skills/system.py|system]] (`src.skills.system`)
- [[nova_backend/src/skills/weather.py|weather]] (`src.skills.weather`)
- [[nova_backend/src/skills/web_search.py|web_search - src/skills]] (`src.skills.web_search`)
- [[nova_backend/src/tasks/notification_schedule_store.py|notification_schedule_store]] (`src.tasks.notification_schedule_store`)
- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/openclaw_execute_executor.py|openclaw_execute_executor]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/identity/nova_self_awareness.py|nova_self_awareness]]
- [[nova_backend/tests/conversation/test_openclaw_agent_personality_bridge.py|test_openclaw_agent_personality_bridge]]
- [[nova_backend/tests/openclaw/test_agent_runner.py|test_agent_runner]]
- [[nova_backend/tests/openclaw/test_agent_runner_goal.py|test_agent_runner_goal]]
- [[nova_backend/tests/openclaw/test_agent_runner_inbox_check_guard.py|test_agent_runner_inbox_check_guard]]
- [[nova_backend/tests/openclaw/test_agent_runtime_store.py|test_agent_runtime_store]]
- [[nova_backend/tests/openclaw/test_agent_scheduler.py|test_agent_scheduler]]
- [[nova_backend/tests/openclaw/test_execution_memory.py|test_execution_memory]]
- [[nova_backend/tests/openclaw/test_per_tool_budget.py|test_per_tool_budget]]
- [[nova_backend/tests/openclaw/test_robust_executor.py|test_robust_executor]]
- [[nova_backend/tests/openclaw/test_strict_preflight.py|test_strict_preflight]]
- [[nova_backend/tests/openclaw/test_task_envelope.py|test_task_envelope]]
- _…and 6 more_

### Tests

- [[nova_backend/tests/openclaw/test_agent_runner.py|test_agent_runner]]
- [[nova_backend/tests/openclaw/test_agent_runtime_store.py|test_agent_runtime_store]]
- [[nova_backend/tests/openclaw/test_agent_scheduler.py|test_agent_scheduler]]
- [[nova_backend/tests/openclaw/test_execution_memory.py|test_execution_memory]]
- [[nova_backend/tests/openclaw/test_per_tool_budget.py|test_per_tool_budget]]
- [[nova_backend/tests/openclaw/test_robust_executor.py|test_robust_executor]]
- [[nova_backend/tests/openclaw/test_strict_preflight.py|test_strict_preflight]]
- [[nova_backend/tests/openclaw/test_task_envelope.py|test_task_envelope]]
- [[nova_backend/tests/openclaw/test_thinking_loop.py|test_thinking_loop]]
- [[nova_backend/tests/openclaw/test_tool_chain.py|test_tool_chain]]
- [[nova_backend/tests/openclaw/test_tool_registry.py|test_tool_registry]]
- [[nova_backend/tests/openclaw/test_user_tool_permissions.py|test_user_tool_permissions]]

## `patterns` (2 files)

### Files

- [[nova_backend/src/patterns/__init__.py|__init__ - src/patterns]]
- [[nova_backend/src/patterns/pattern_review_store.py|pattern_review_store - src/patterns]]
    - def _utc_now() -> datetime:

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/working_context/pattern_review_store.py|pattern_review_store - src/working_context]]
- [[nova_backend/tests/phase5/test_pattern_review_store.py|test_pattern_review_store]]

### Tests

- [[nova_backend/tests/phase5/test_pattern_review_store.py|test_pattern_review_store]]

## `perception` (6 files)

___all__ = [_

### Files

- [[nova_backend/src/perception/__init__.py|__init__ - src/perception]]
- [[nova_backend/src/perception/cursor_locator.py|cursor_locator]]
    - def locate_cursor() -> dict[str, Any]:
- [[nova_backend/src/perception/explain_anything.py|explain_anything]]
    - @dataclass(frozen=True)
- [[nova_backend/src/perception/ocr_pipeline.py|ocr_pipeline]]
    - class OCRPipeline:
- [[nova_backend/src/perception/screen_capture.py|screen_capture]]
    - DEFAULT_CAPTURE_DIR = Path(__file__).resolve().parents[1] / "data" / "captures
- [[nova_backend/src/perception/vision_analyzer.py|vision_analyzer]]
    - class VisionAnalyzer:

### Imported by

- [[nova_backend/src/api/live_screen_api.py|live_screen_api]]
- [[nova_backend/src/context/context_snapshot_service.py|context_snapshot_service]]
- [[nova_backend/src/executors/explain_anything_executor.py|explain_anything_executor]]
- [[nova_backend/src/executors/screen_analysis_executor.py|screen_analysis_executor]]
- [[nova_backend/src/executors/screen_capture_executor.py|screen_capture_executor]]
- [[nova_backend/tests/phase45/test_explain_anything_router.py|test_explain_anything_router]]
- [[nova_backend/tests/phase45/test_vision_analyzer.py|test_vision_analyzer]]

### Tests

- [[nova_backend/tests/phase45/test_vision_analyzer.py|test_vision_analyzer]]

## `personality` (10 files)

___all__ = [_

### Files

- [[nova_backend/src/personality/__init__.py|__init__ - src/personality]]
- [[nova_backend/src/personality/announce.py|announce]]
    - def deep_mode_activation_notice() -> str:
- [[nova_backend/src/personality/conversation_personality_agent.py|conversation_personality_agent]]
    - class ConversationPersonalityAgent:
- [[nova_backend/src/personality/core.py|core]]
    - class PersonalityAgent:
- [[nova_backend/src/personality/deep_mode.py|deep_mode]]
    - @dataclass
- [[nova_backend/src/personality/interface_agent.py|interface_agent]]
    - class PersonalityInterfaceAgent:
- [[nova_backend/src/personality/nova_style_contract.py|nova_style_contract]]
    - class NovaStyleContract:
- [[nova_backend/src/personality/presenter.py|presenter]]
    - def present_raw_outputs(outputs: Iterable[AgentOutput]) -> str:
- [[nova_backend/src/personality/tone_profile_store.py|tone_profile_store]]
    - def _utc_now() -> str:
- [[nova_backend/src/personality/validate.py|validate]]
    - class PersonalityValidator:

### Imports from

- [[nova_backend/src/agents/base.py|base]] (`src.agents.base`)
- [[nova_backend/src/build_phase.py|build_phase]] (`src.build_phase`)
- [[nova_backend/src/governor/agent_orchestrator.py|agent_orchestrator - src/governor]] (`src.governor.agent_orchestrator`)
- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)
- [[nova_backend/src/validation/pipeline.py|pipeline]] (`src.validation.pipeline`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/conversation/general_chat_runtime.py|general_chat_runtime]]
- [[nova_backend/src/conversation/response_formatter.py|response_formatter]]
- [[nova_backend/src/conversation/session_router.py|session_router]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/openclaw/agent_personality_bridge.py|agent_personality_bridge]]
- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]]
- [[nova_backend/src/skills/general_chat.py|general_chat]]
- [[nova_backend/src/voice/voice_agent.py|voice_agent]]
- [[nova_backend/tests/conversation/test_conversation_personality_agent.py|test_conversation_personality_agent]]
- [[nova_backend/tests/conversation/test_general_chat_tone.py|test_general_chat_tone]]
- [[nova_backend/tests/conversation/test_personality_interface_agent.py|test_personality_interface_agent]]
- [[nova_backend/tests/phase42/test_phase42_runtime_lock.py|test_phase42_runtime_lock]]
- [[nova_backend/tests/phase5/test_tone_profile_store.py|test_tone_profile_store]]

### Tests

- [[nova_backend/tests/conversation/test_conversation_personality_agent.py|test_conversation_personality_agent]]
- [[nova_backend/tests/phase5/test_tone_profile_store.py|test_tone_profile_store]]

## `policies` (3 files)

___all__ = ["AtomicPolicyStore", "PolicyValidationResult", "PolicyValidator"]_

### Files

- [[nova_backend/src/policies/__init__.py|__init__ - src/policies]]
- [[nova_backend/src/policies/atomic_policy_store.py|atomic_policy_store]]
    - def _utc_now() -> str:
- [[nova_backend/src/policies/policy_validator.py|policy_validator]]
    - _WEEKDAY_CODES = {"MO", "TU", "WE", "TH", "FR", "SA", "SU"}

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/governor/governor.py|governor]]
- [[nova_backend/tests/phase6/test_atomic_policy_store.py|test_atomic_policy_store]]
- [[nova_backend/tests/phase6/test_policy_executor_gate.py|test_policy_executor_gate]]
- [[nova_backend/tests/phase6/test_policy_validator.py|test_policy_validator]]

### Tests

- [[nova_backend/tests/phase6/test_atomic_policy_store.py|test_atomic_policy_store]]
- [[nova_backend/tests/phase6/test_policy_validator.py|test_policy_validator]]

## `profiles` (2 files)

### Files

- [[nova_backend/src/profiles/__init__.py|__init__ - src/profiles]]
- [[nova_backend/src/profiles/user_profile_store.py|user_profile_store]]
    - _ALLOWED_STYLES = {"concise", "balanced", "detailed"}

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/api/profile_api.py|profile_api]]
- [[nova_backend/tests/phase45/test_profile_api.py|test_profile_api]]
- [[nova_backend/tests/phase45/test_user_profile_store.py|test_user_profile_store]]

### Tests

- [[nova_backend/tests/phase45/test_user_profile_store.py|test_user_profile_store]]

## `prompts` (1 files)

### Files

- [[nova_backend/src/prompts/research_prompt.py|research_prompt]]
    - RESEARCH_SUMMARIZE_SYSTEM = (

## `providers` (2 files)

_Provider lanes for optional metered model access._

### Files

- [[nova_backend/src/providers/__init__.py|__init__ - src/providers]]
- [[nova_backend/src/providers/openai_responses_lane.py|openai_responses_lane]]
    - class OpenAIResponsesLaneError(RuntimeError):

### Imports from

- [[nova_backend/src/governor/network_mediator.py|network_mediator]] (`src.governor.network_mediator`)
- [[nova_backend/src/settings/runtime_settings_store.py|runtime_settings_store]] (`src.settings.runtime_settings_store`)
- [[nova_backend/src/usage/provider_usage_store.py|provider_usage_store]] (`src.usage.provider_usage_store`)

### Imported by

- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]]
- [[nova_backend/tests/openclaw/test_openai_responses_lane.py|test_openai_responses_lane]]

### Tests

- [[nova_backend/tests/openclaw/test_openai_responses_lane.py|test_openai_responses_lane]]

## `rendering` (3 files)

### Files

- [[nova_backend/src/rendering/__init__.py|__init__ - src/rendering]]
- [[nova_backend/src/rendering/intelligence_brief_renderer.py|intelligence_brief_renderer]]
    - class IntelligenceBriefRenderer:
- [[nova_backend/src/rendering/speech_formatter.py|speech_formatter]]
    - class SpeechFormatter:

### Imported by

- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]]
- [[nova_backend/src/executors/news_intelligence_executor.py|news_intelligence_executor]]
- [[nova_backend/src/executors/tts_executor.py|tts_executor]]
- [[nova_backend/src/voice/tts_engine.py|tts_engine]]
- [[nova_backend/tests/rendering/test_intelligence_brief_renderer.py|test_intelligence_brief_renderer]]
- [[nova_backend/tests/rendering/test_speech_formatter.py|test_speech_formatter]]

### Tests

- [[nova_backend/tests/rendering/test_intelligence_brief_renderer.py|test_intelligence_brief_renderer]]
- [[nova_backend/tests/rendering/test_speech_formatter.py|test_speech_formatter]]

## `routers` (1 files)

### Files

- [[nova_backend/src/routers/stt.py|stt]]
    - Phase-3 STT Router (freeze-ready)

### Imports from

- [[nova_backend/src/services/stt_engine.py|stt_engine]] (`src.services.stt_engine`)

### Imported by

- [[docs/archive/phase 3.5/brain_server_Phase3.py|brain_server_Phase3 - archive/phase 3.5]]
- [[docs/archive/phase 3/brain_server_Phase3.py|brain_server_Phase3 - archive/phase 3]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/tests/test_stt_engine.py|test_stt_engine]]

## `services` (2 files)

### Files

- [[nova_backend/src/services/stt_engine.py|stt_engine]]
    - Phase-3 STT Engine (LOCAL, INPUT-ONLY, FREEZE-READY)
- [[nova_backend/src/services/weather_service.py|weather_service]]
    - NETWORK_CAPABILITY_ID = 55

### Imports from

- [[nova_backend/src/governor/exceptions.py|exceptions]] (`src.governor.exceptions`)
- [[nova_backend/src/governor/network_mediator.py|network_mediator]] (`src.governor.network_mediator`)

### Imported by

- [[nova_backend/src/routers/stt.py|stt]]
- [[nova_backend/src/skills/weather.py|weather]]
- [[nova_backend/src/voice/tts_engine.py|tts_engine]]
- [[nova_backend/tests/test_weather_service.py|test_weather_service]]
- [[nova_backend/tests/test_weather_skill.py|test_weather_skill]]

### Tests

- [[nova_backend/tests/test_stt_engine.py|test_stt_engine]]
- [[nova_backend/tests/test_weather_service.py|test_weather_service]]

## `settings` (2 files)

___all__ = ["RuntimeSettingsStore", "runtime_settings_store"]_

### Files

- [[nova_backend/src/settings/__init__.py|__init__ - src/settings]]
- [[nova_backend/src/settings/runtime_settings_store.py|runtime_settings_store]]
    - def _utc_now() -> str:

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/external_reasoning_executor.py|external_reasoning_executor]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/identity/nova_self_awareness.py|nova_self_awareness]]
- [[nova_backend/src/openclaw/agent_runtime_store.py|agent_runtime_store]]
- [[nova_backend/src/openclaw/agent_scheduler.py|agent_scheduler]]
- [[nova_backend/src/providers/openai_responses_lane.py|openai_responses_lane]]
- [[nova_backend/tests/openclaw/test_agent_scheduler.py|test_agent_scheduler]]
- [[nova_backend/tests/openclaw/test_openai_responses_lane.py|test_openai_responses_lane]]
- [[nova_backend/tests/phase9/test_token_awareness.py|test_token_awareness]]
- [[nova_backend/tests/test_openclaw_agent_api.py|test_openclaw_agent_api]]
- [[nova_backend/tests/test_runtime_settings_api.py|test_runtime_settings_api]]

## `skill_registry` (1 files)

### Files

- [[nova_backend/src/skill_registry.py|skill_registry]]
    - NovaLIS deterministic skill registry.

### Imports from

- [[nova_backend/src/base_skill.py|base_skill]] (`src.base_skill`)
- [[nova_backend/src/governor/network_mediator.py|network_mediator]] (`src.governor.network_mediator`)
- [[nova_backend/src/skills/calendar.py|calendar]] (`src.skills.calendar`)
- [[nova_backend/src/skills/general_chat.py|general_chat]] (`src.skills.general_chat`)
- [[nova_backend/src/skills/news.py|news]] (`src.skills.news`)
- [[nova_backend/src/skills/system.py|system]] (`src.skills.system`)
- [[nova_backend/src/skills/weather.py|weather]] (`src.skills.weather`)

### Imported by

- [[nova_backend/tests/simulation/conversation_simulator.py|conversation_simulator]]

## `skills` (8 files)

_NovaLIS Backend Package_

### Files

- [[nova_backend/src/skills/__init__.py|__init__ - src/skills]]
- [[nova_backend/src/skills/calendar.py|calendar]]
    - class CalendarSkill(BaseSkill):
- [[nova_backend/src/skills/executor_adapter.py|executor_adapter]]
    - Generic adapter: wraps any Governor executor into BaseSkill/SkillResult.
- [[nova_backend/src/skills/general_chat.py|general_chat]]
    - NovaLIS General Chat Skill — Phase 3 SAFE / Phase 4.2 cognitive staging.
- [[nova_backend/src/skills/news.py|news]]
    - NovaLIS News Skill — Phase 3 Canonical
- [[nova_backend/src/skills/system.py|system]]
    - System Skill — Phase 3
- [[nova_backend/src/skills/weather.py|weather]]
    - log = logging.getLogger("nova.skills.weather")
- [[nova_backend/src/skills/web_search.py|web_search - src/skills]]
    - Web search skill for OpenClaw — wraps Governor capability 16.

### Imports from

- [[nova_backend/src/base_skill.py|base_skill]] (`src.base_skill`)
- [[nova_backend/src/conversation/complexity_heuristics.py|complexity_heuristics]] (`src.conversation.complexity_heuristics`)
- [[nova_backend/src/conversation/deepseek_bridge.py|deepseek_bridge]] (`src.conversation.deepseek_bridge`)
- [[nova_backend/src/conversation/deepseek_safety_wrapper.py|deepseek_safety_wrapper]] (`src.conversation.deepseek_safety_wrapper`)
- [[nova_backend/src/conversation/escalation_policy.py|escalation_policy]] (`src.conversation.escalation_policy`)
- [[nova_backend/src/conversation/response_formatter.py|response_formatter]] (`src.conversation.response_formatter`)
- [[nova_backend/src/conversation/response_style_router.py|response_style_router]] (`src.conversation.response_style_router`)
- [[nova_backend/src/conversation/safety_filter.py|safety_filter]] (`src.conversation.safety_filter`)
- [[nova_backend/src/executors/web_search_executor.py|web_search_executor]] (`src.executors.web_search_executor`)
- [[nova_backend/src/governor/execute_boundary/__init__.py|__init__ - governor/execute_boundary]] (`src.governor.execute_boundary`)
- [[nova_backend/src/governor/network_mediator.py|network_mediator]] (`src.governor.network_mediator`)
- [[nova_backend/src/identity/nova_self_awareness.py|nova_self_awareness]] (`src.identity.nova_self_awareness`)
- [[nova_backend/src/llm/llm_gateway.py|llm_gateway]] (`src.llm.llm_gateway`)
- [[nova_backend/src/memory/nova_self_memory_store.py|nova_self_memory_store]] (`src.memory.nova_self_memory_store`)
- [[nova_backend/src/memory/user_memory_store.py|user_memory_store]] (`src.memory.user_memory_store`)
- [[nova_backend/src/personality/nova_style_contract.py|nova_style_contract]] (`src.personality.nova_style_contract`)
- [[nova_backend/src/personality/tone_profile_store.py|tone_profile_store]] (`src.personality.tone_profile_store`)
- [[nova_backend/src/services/weather_service.py|weather_service]] (`src.services.weather_service`)
- [[nova_backend/src/tools/news_fallback.py|news_fallback]] (`src.tools.news_fallback`)
- [[nova_backend/src/tools/rss_fetch.py|rss_fetch]] (`src.tools.rss_fetch`)

### Imported by

- [[nova_backend/src/api/openclaw_agent_api.py|openclaw_agent_api]]
- [[nova_backend/src/conversation/general_chat_runtime.py|general_chat_runtime]]
- [[nova_backend/src/executors/info_snapshot_executor.py|info_snapshot_executor]]
- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]]
- [[nova_backend/src/openclaw/tool_registry.py|tool_registry]]
- [[nova_backend/src/skill_registry.py|skill_registry]]
- [[nova_backend/tests/conversation/test_general_chat_deep_thought.py|test_general_chat_deep_thought]]
- [[nova_backend/tests/conversation/test_general_chat_tone.py|test_general_chat_tone]]
- [[nova_backend/tests/governance/test_skills_use_network_mediator_only.py|test_skills_use_network_mediator_only]]
- [[nova_backend/tests/openclaw/test_executor_adapter.py|test_executor_adapter]]
- [[nova_backend/tests/openclaw/test_web_search_skill.py|test_web_search_skill]]
- [[nova_backend/tests/phase45/test_brain_server_followups_and_voice.py|test_brain_server_followups_and_voice]]
- [[nova_backend/tests/test_calendar_skill.py|test_calendar_skill]]
- [[nova_backend/tests/test_news_skill.py|test_news_skill]]
- [[nova_backend/tests/test_weather_skill.py|test_weather_skill]]

### Tests

- [[nova_backend/tests/openclaw/test_executor_adapter.py|test_executor_adapter]]

## `speech_state` (1 files)

### Files

- [[nova_backend/src/speech_state.py|speech_state]]
    - Speech State - Phase-2+

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/tts_executor.py|tts_executor]]
- [[nova_backend/src/voice/tts_engine.py|tts_engine]]

## `system_control` (2 files)

_System control boundary package._

### Files

- [[nova_backend/src/system_control/__init__.py|__init__ - src/system_control]]
- [[nova_backend/src/system_control/system_control_executor.py|system_control_executor]]
    - class SystemControlExecutor:

### Imported by

- [[nova_backend/src/executors/brightness_executor.py|brightness_executor]]
- [[nova_backend/src/executors/media_executor.py|media_executor]]
- [[nova_backend/src/executors/open_folder_executor.py|open_folder_executor]]
- [[nova_backend/src/executors/volume_executor.py|volume_executor]]
- [[nova_backend/tests/executors/test_system_control_executor.py|test_system_control_executor - tests/executors]]
- [[nova_backend/tests/test_system_control_executor.py|test_system_control_executor - nova_backend/tests]]

### Tests

- [[nova_backend/tests/executors/test_system_control_executor.py|test_system_control_executor - tests/executors]]
- [[nova_backend/tests/test_system_control_executor.py|test_system_control_executor - nova_backend/tests]]

## `tasks` (3 files)

___all__ = ["ReminderTask", "NotificationScheduleStore"]_

### Files

- [[nova_backend/src/tasks/__init__.py|__init__ - src/tasks]]
- [[nova_backend/src/tasks/notification_schedule_store.py|notification_schedule_store]]
    - def _utc_now() -> datetime:
- [[nova_backend/src/tasks/reminder_task.py|reminder_task]]
    - Reminder Task Schema

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/api/openclaw_agent_api.py|openclaw_agent_api]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/openclaw/agent_runner.py|agent_runner]]
- [[nova_backend/src/openclaw/agent_scheduler.py|agent_scheduler]]
- [[nova_backend/tests/openclaw/test_agent_scheduler.py|test_agent_scheduler]]
- [[nova_backend/tests/phase5/test_notification_schedule_store.py|test_notification_schedule_store]]
- [[nova_backend/tests/phase9/test_token_awareness.py|test_token_awareness]]
- [[nova_backend/tests/test_openclaw_agent_api.py|test_openclaw_agent_api]]

### Tests

- [[nova_backend/tests/phase5/test_notification_schedule_store.py|test_notification_schedule_store]]

## `tools` (4 files)

_NovaLIS Tools Package_

### Files

- [[nova_backend/src/tools/__init__.py|__init__ - src/tools]]
- [[nova_backend/src/tools/news_fallback.py|news_fallback]]
    - Deterministic fallback for NewsSkill when RSS fails.
- [[nova_backend/src/tools/rss_fetch.py|rss_fetch]]
    - NETWORK_CAPABILITY_ID = 56
- [[nova_backend/src/tools/web_search.py|web_search - src/tools]]
    - NovaLIS Legacy Web Search Tool (Sealed)

### Imports from

- [[nova_backend/src/governor/exceptions.py|exceptions]] (`src.governor.exceptions`)
- [[nova_backend/src/governor/network_mediator.py|network_mediator]] (`src.governor.network_mediator`)

### Imported by

- [[nova_backend/src/skills/news.py|news]]
- [[nova_backend/tests/test_rss_fetch.py|test_rss_fetch]]

### Tests

- [[nova_backend/tests/test_rss_fetch.py|test_rss_fetch]]

## `trust` (3 files)

___all__ = ["FailureLadder", "FailureLadderThresholds", "normalize_trust_status"]_

### Files

- [[nova_backend/src/trust/__init__.py|__init__ - src/trust]]
- [[nova_backend/src/trust/failure_ladder.py|failure_ladder]]
    - @dataclass(frozen=True)
- [[nova_backend/src/trust/trust_contract.py|trust_contract]]
    - ALLOWED_MODES = {"Local-only", "Online"}

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/tests/phase45/test_failure_ladder.py|test_failure_ladder]]
- [[nova_backend/tests/phase45/test_trust_contract.py|test_trust_contract]]

### Tests

- [[nova_backend/tests/phase45/test_failure_ladder.py|test_failure_ladder]]
- [[nova_backend/tests/phase45/test_trust_contract.py|test_trust_contract]]

## `usage` (2 files)

___all__ = ["ProviderUsageStore", "provider_usage_store"]_

### Files

- [[nova_backend/src/usage/__init__.py|__init__ - src/usage]]
- [[nova_backend/src/usage/provider_usage_store.py|provider_usage_store]]
    - def _utc_now() -> str:

### Imports from

- [[nova_backend/src/utils/persistent_state.py|persistent_state]] (`src.utils.persistent_state`)

### Imported by

- [[nova_backend/src/api/settings_api.py|settings_api]]
- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/conversation/deepseek_bridge.py|deepseek_bridge]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/governor/governor.py|governor]]
- [[nova_backend/src/providers/openai_responses_lane.py|openai_responses_lane]]
- [[nova_backend/tests/conversation/test_deepseek_bridge.py|test_deepseek_bridge]]
- [[nova_backend/tests/conversation/test_deepseek_usage_visibility.py|test_deepseek_usage_visibility]]
- [[nova_backend/tests/conversation/test_provider_usage_store.py|test_provider_usage_store]]
- [[nova_backend/tests/openclaw/test_openai_responses_lane.py|test_openai_responses_lane]]
- [[nova_backend/tests/phase7/test_phase7_runtime_contract.py|test_phase7_runtime_contract]]
- [[nova_backend/tests/phase9/test_token_awareness.py|test_token_awareness]]
- [[nova_backend/tests/test_openclaw_agent_api.py|test_openclaw_agent_api]]

### Tests

- [[nova_backend/tests/conversation/test_provider_usage_store.py|test_provider_usage_store]]

## `utils` (6 files)

### Files

- [[nova_backend/src/utils/content_extractor.py|content_extractor]]
    - def extract_text_from_html(html: str, max_chars: int = 5000) -> str:
- [[nova_backend/src/utils/date_extractor.py|date_extractor]]
    - def extract_publish_date(html: str) -> Optional[datetime]:
- [[nova_backend/src/utils/local_request_guard.py|local_request_guard]]
    - _ALLOWED_LOOPBACK_HOSTS = {
- [[nova_backend/src/utils/path_resolver.py|path_resolver]]
    - Local path resolution and project exploration utilities for the Nova brain server.
- [[nova_backend/src/utils/persistent_state.py|persistent_state]]
    - _REGISTRY_LOCK = Lock()
- [[nova_backend/src/utils/web_target_planner.py|web_target_planner - src/utils]]
    - Presets and aliases that can open directly without confirmation.

### Imports from

- [[nova_backend/src/websocket/intent_patterns.py|intent_patterns]] (`src.websocket.intent_patterns`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/connections/connections_store.py|connections_store]]
- [[nova_backend/src/executors/news_intelligence_executor.py|news_intelligence_executor]]
- [[nova_backend/src/executors/web_search_executor.py|web_search_executor]]
- [[nova_backend/src/executors/webpage_launch_executor.py|webpage_launch_executor]]
- [[nova_backend/src/governor/web_target_planner.py|web_target_planner - src/governor]]
- [[nova_backend/src/memory/governed_memory_store.py|governed_memory_store]]
- [[nova_backend/src/memory/nova_self_memory_store.py|nova_self_memory_store]]
- [[nova_backend/src/memory/user_memory_store.py|user_memory_store]]
- [[nova_backend/src/openclaw/agent_runtime_store.py|agent_runtime_store]]
- [[nova_backend/src/patterns/pattern_review_store.py|pattern_review_store - src/patterns]]
- [[nova_backend/src/personality/tone_profile_store.py|tone_profile_store]]
- [[nova_backend/src/policies/atomic_policy_store.py|atomic_policy_store]]
- [[nova_backend/src/profiles/user_profile_store.py|user_profile_store]]
- [[nova_backend/src/settings/runtime_settings_store.py|runtime_settings_store]]
- _…and 3 more_

## `validation` (6 files)

___all__ = [_

### Files

- [[nova_backend/src/validation/__init__.py|__init__ - src/validation]]
- [[nova_backend/src/validation/authority.py|authority]]
    - @dataclass(frozen=True)
- [[nova_backend/src/validation/emotional.py|emotional]]
    - @dataclass(frozen=True)
- [[nova_backend/src/validation/formatting.py|formatting]]
    - @dataclass(frozen=True)
- [[nova_backend/src/validation/pipeline.py|pipeline]]
    - @dataclass(frozen=True)
- [[nova_backend/src/validation/transformation.py|transformation]]
    - @dataclass(frozen=True)

### Imported by

- [[nova_backend/src/executors/multi_source_reporting_executor.py|multi_source_reporting_executor]]
- [[nova_backend/src/personality/validate.py|validate]]
- [[nova_backend/tests/phase42/test_validation_pipeline.py|test_validation_pipeline]]

## `voice` (4 files)

_Voice-layer helpers for Tier-B conversational responsiveness._

### Files

- [[nova_backend/src/voice/__init__.py|__init__ - src/voice]]
- [[nova_backend/src/voice/stt_pipeline.py|stt_pipeline]]
    - @dataclass(frozen=True)
- [[nova_backend/src/voice/tts_engine.py|tts_engine]]
    - logger = logging.getLogger(__name__)
- [[nova_backend/src/voice/voice_agent.py|voice_agent]]
    - class VoiceExperienceAgent:

### Imports from

- [[nova_backend/src/audio/audio_task_runner.py|audio_task_runner]] (`src.audio.audio_task_runner`)
- [[nova_backend/src/conversation/response_formatter.py|response_formatter]] (`src.conversation.response_formatter`)
- [[nova_backend/src/executors/tts_executor.py|tts_executor]] (`src.executors.tts_executor`)
- [[nova_backend/src/governor/exceptions.py|exceptions]] (`src.governor.exceptions`)
- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/personality/nova_style_contract.py|nova_style_contract]] (`src.personality.nova_style_contract`)
- [[nova_backend/src/rendering/speech_formatter.py|speech_formatter]] (`src.rendering.speech_formatter`)
- [[nova_backend/src/services/stt_engine.py|stt_engine]] (`src.services.stt_engine`)
- [[nova_backend/src/speech_state.py|speech_state]] (`src.speech_state`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/executors/os_diagnostics_executor.py|os_diagnostics_executor]]
- [[nova_backend/src/executors/tts_executor.py|tts_executor]]
- [[nova_backend/tests/conversation/test_voice_agent.py|test_voice_agent]]
- [[nova_backend/tests/rendering/test_tts_engine.py|test_tts_engine]]

### Tests

- [[nova_backend/tests/conversation/test_voice_agent.py|test_voice_agent]]
- [[nova_backend/tests/rendering/test_tts_engine.py|test_tts_engine]]

## `websocket` (3 files)

_Websocket session runtime modules._

### Files

- [[nova_backend/src/websocket/__init__.py|__init__ - src/websocket]]
- [[nova_backend/src/websocket/intent_patterns.py|intent_patterns]]
    - Intent recognition patterns for the Nova brain server session loop.
- [[nova_backend/src/websocket/session_handler.py|session_handler]]
    - async def run_websocket_session(ws: WebSocket, deps: Any) -> None:

### Imports from

- [[nova_backend/src/agents/adversarial.py|adversarial]] (`src.agents.adversarial`)
- [[nova_backend/src/agents/architect.py|architect]] (`src.agents.architect`)
- [[nova_backend/src/agents/assumption.py|assumption]] (`src.agents.assumption`)
- [[nova_backend/src/agents/builder.py|builder]] (`src.agents.builder`)
- [[nova_backend/src/agents/contradiction.py|contradiction]] (`src.agents.contradiction`)
- [[nova_backend/src/agents/deep_audit.py|deep_audit]] (`src.agents.deep_audit`)
- [[nova_backend/src/agents/memory.py|memory - src/agents]] (`src.agents.memory`)
- [[nova_backend/src/build_phase.py|build_phase]] (`src.build_phase`)
- [[nova_backend/src/llm/llm_gateway.py|llm_gateway]] (`src.llm.llm_gateway`)
- [[nova_backend/src/memory/governed_memory_store.py|governed_memory_store]] (`src.memory.governed_memory_store`)
- [[nova_backend/src/utils/local_request_guard.py|local_request_guard]] (`src.utils.local_request_guard`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/utils/path_resolver.py|path_resolver]]

## `working_context` (11 files)

_build_operational_context_widget,_

### Files

- [[nova_backend/src/working_context/__init__.py|__init__ - src/working_context]]
- [[nova_backend/src/working_context/assistive_noticing.py|assistive_noticing]]
    - def _utc_now() -> datetime:
- [[nova_backend/src/working_context/context_builder.py|context_builder]]
    - infer_goal_from_turn,
- [[nova_backend/src/working_context/context_pruner.py|context_pruner]]
    - MAX_TURN_HISTORY = 2
- [[nova_backend/src/working_context/context_router.py|context_router]]
    - class WorkingContextRouter:
- [[nova_backend/src/working_context/context_signals.py|context_signals]]
    - def infer_task_type(text: str, *, intent_family: str = "") -> str:
- [[nova_backend/src/working_context/context_state.py|context_state]]
    - @dataclass(frozen=True)
- [[nova_backend/src/working_context/context_store.py|context_store]]
    - class WorkingContextStore:
- [[nova_backend/src/working_context/operational_remembrance.py|operational_remembrance]]
    - def _clean(value: Any) -> str:
- [[nova_backend/src/working_context/pattern_review_store.py|pattern_review_store - src/working_context]]
    - __all__ = ['PatternReviewStore']
- [[nova_backend/src/working_context/project_threads.py|project_threads]]
    - def _now_iso() -> str:

### Imports from

- [[nova_backend/src/ledger/writer.py|writer]] (`src.ledger.writer`)
- [[nova_backend/src/patterns/pattern_review_store.py|pattern_review_store - src/patterns]] (`src.patterns.pattern_review_store`)

### Imported by

- [[nova_backend/src/brain_server.py|brain_server]]
- [[nova_backend/src/conversation/general_chat_runtime.py|general_chat_runtime]]
- [[nova_backend/tests/phase45/test_working_context_store.py|test_working_context_store - tests/phase45]]
- [[nova_backend/tests/phase5/test_operational_remembrance.py|test_operational_remembrance]]
- [[nova_backend/tests/phase5/test_project_thread_store.py|test_project_thread_store]]
- [[nova_backend/tests/phase5/test_working_context_store.py|test_working_context_store - tests/phase5]]
- [[nova_backend/tests/phase8/test_assistive_noticing.py|test_assistive_noticing]]

### Tests

- [[nova_backend/tests/phase5/test_operational_remembrance.py|test_operational_remembrance]]
- [[nova_backend/tests/phase5/test_pattern_review_store.py|test_pattern_review_store]]
- [[nova_backend/tests/phase8/test_assistive_noticing.py|test_assistive_noticing]]
